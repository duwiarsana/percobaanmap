...
const MapController: React.FC<{
  ...
  // Fungsi khusus untuk memuat data kecamatan Banyuwangi
  const loadBanyuwangiSubdistrictData = async (districtId: string | number) => {
    console.log(`Loading Banyuwangi subdistrict data with ID: ${districtId}`);
    try {
      // Define the kecamatan files for Banyuwangi
      const kecamatanFiles = [
        "id3510010_pesanggaran",
        "id3510011_siliragung",
        "id3510020_bangorejo",
        "id3510030_purwoharjo",
        "id3510040_tegaldlimo",
        "id3510050_muncar",
        "id3510060_cluring",
        "id3510070_gambiran",
        "id3510071_tegalsari",
        ...
        "id3510110_srono",
        "id3510120_rogojampi",
        "id3510121_blimbingsari",
        "id3510130_kabat",
        "id3510140_singojuruh",
        "id3510150_sempu",
        "id3510160_songgon",
        "id3510170_glagah",
        "id3510171_licin",
        "id3510180_banyuwangi",
        "id3510190_giri",
        "id3510200_kalipuro",
        "id3510210_wongsorejo"
      ];
      ...
      if (combinedFeatures.length > 0) {
        ...
        setSubdistrictData(combinedData);
        console.log(`Successfully loaded ${combinedFeatures.length} kecamatan features for Banyuwangi from ${loadedCount} files`);
        return Promise.resolve();
      }
      ...
      if (!response.ok) throw new Error('Banyuwangi district file not found');
      const text = await response.text();
      ...
      const data = JSON.parse(text);
      setSubdistrictData(data);
      console.log('Successfully loaded Banyuwangi district data');
      return Promise.resolve();
    } catch (error) {
      console.error("Error loading Banyuwangi subdistrict data:", error);
      // Set empty GeoJSON to avoid crashes
      setSubdistrictData({
        type: "FeatureCollection",
        features: []
  ...
  const loadSubdistrictData = async (districtId: string | number) => {
    ...
    // Check if this is the Banyuwangi UUID
    if (districtId.toString() === "5ae5b33d-9074-49ea-9d7a-7ceb6738cce5") {
      console.log("Detected Banyuwangi UUID, redirecting to Banyuwangi handler");
      return loadBanyuwangiSubdistrictData(districtId);
    }
